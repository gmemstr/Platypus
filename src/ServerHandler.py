...
class Sql:
	...
	def Verify(self,uid):
		...
		if dbdata is None: return False
		if dbdata == uid: return True
		else: return False
	...
	def Get(self,id=""):
		...
		self.c.execute("SELECT id,name,hostname FROM %s WHERE id=%s",(self.sqltable,id))
		dbdata = self.c.fetchone()
	...
	def UuidToId(self,uuid):
		...
		self.c.execute("SELECT id FROM %s WHERE uuid=%s",(self.sqltable,uuid))
		dbdata = self.c.fetchone()
...
class Cache:
	def __init__(self):
		with open("cache/data.json") as data_file:
			self.cache = data_file
